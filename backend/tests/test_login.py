...
class TestLogin(flask_unittest.ClientTestCase):
    ...
    def test_login_missing_key(self, client):
        response = client.post(
            "/login/agent", json=dict(booking_agent_id="1", password="best123")
        )
        self.assertEqual(
            dict(result="error", message='Missing required key "email"!', key="email"),
            response.json,
        )
    ...
    def test_login_wrong_password(self, client):
        response = client.post(
            "/login/staff", json=dict(username="staffnumberone", password="wendy2")
        )
        self.assertEqual(
            dict(
                result="error",
    ...
    def test_login_non_existence_user(self, client):
        response = client.post(
            "/login/cust", json=dict(email="speiaz123@nyu2.edu", password="wendy")
        )
        self.assertEqual(
            dict(
                result="error",
                message="The input information doesn't match any existing users!",
            ),
            response.json,
        )
